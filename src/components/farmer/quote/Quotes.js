...
export default class Quotes extends Component {
  ...
  componentDidMount() {
    axios
      .get(
        `${process.env.REACT_APP_API_URL}/quotes?farmer=${this.props.user._id}`,
      ...
      <Container>
              ...
              {this.state.quotes.map((quote) => {
                return (
                  <Card
                    key={quote._id}
                    border="primary"
                    style={{ width: "18rem", borderRadius: "4px" }}
                  >
                    <Card.Body>
                      <Card.Title>{quote.field.fieldName}</Card.Title>
                      <Card.Subtitle className="mb-2 text-muted">
                        Number of offers: {quote.offers.length}
                      </Card.Subtitle>
                      <Card.Text>
                      ...
                      <Link to={`/dashboard/quotes/${quote._id}`}>
                        More details
                      </Link>
                    </Card.Body>
                  </Card>
        ...
        <Row>
          {this.state.showAddForm ? (
            <AddQuote onAddQuote={this.handleAddQuote} />
          ) : (
